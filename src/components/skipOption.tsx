  ...
  return (
    <label
      ...
      className={cn("border-2 p-6 rounded-lg transition-all", {
        ...
        "bg-[#C1C1C1]/10 border-[#2A2A2A]": !checked,
      })}
    >
      <input
        id={id}
...
export const SkipOptionWithImage = (props: Props) => {
  const { id, name, value, checked, option, onChange } = props;
  ...
  return (
    <label
      htmlFor={id}
      className={`border-2 p-6 rounded-lg transition-all ${checked ? "" : "bg-[#C1C1C1]/10 border-[#2A2A2A]"}`}
    >
      <input
        id={id}
        type="radio"
